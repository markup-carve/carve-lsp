#!/usr/bin/env node
// A git dependency ships to every consumer of this package.
//
//   node tools/no-git-dependencies.mjs [package.json]
//
...
// `github:` or `git+` spec in `dependencies` means every install of the language
// server clones carve-js over git. Three consequences, all borne by the
// consumer: the install needs git and GitHub reachability (and resolves over SSH
// wherever an `insteadOf` rewrite is configured), npm has no registry tarball to
// check an integrity hash against, and a commit pin stops tracking the engine's
// releases so a consumer on a caret range still cannot receive an engine fix.
//
// This is a REGRESSION guard, not a hypothetical. The engine was declared as a
// registry version at tag v0.1.3 - whose release notes state that consumers no
// longer need Git to install the server - and a pin-moving pull request put it
// back to `github:markup-carve/carve-js#<sha>` three and a half hours after the
// tag, with nothing to notice. markup-carve/carve-grammars#276 added the same
// check there after the same regression reached the registry.
//
// `dependencies`, `optionalDependencies` and `peerDependencies` are inspected,
// because those are the three a consumer installs - npm fetches an optional
// dependency like any other and only tolerates its failure afterwards, so a git
// spec there costs the consumer the same git clone. A git `devDependency` costs
// a contributor a git clone and costs a consumer nothing, so it is left alone.
//
...
// in this org: `git+https://` in pandoc-carve, `github:` in carve-lsp, and npm's
// bare `owner/repo#ref` shorthand, which carries no protocol at all. Each fix
// added the next spelling to the list, which is why there was a next one. So the
// final test below is a CATCH-ALL over the characters a registry range cannot
// contain: whatever spelling comes after this one, it does not have to be
// anticipated to be caught.
//
...
// contract all the same (#126, markup-carve/carve-grammars#302).
//
...
// npa agree on 114. Before #126 they agreed on 65.
//
...
// spelling this file was accepting (#123). Named separately from the catch-all
...
// `codex review` on #126, one layer under the alias hole it was reviewing.
...
function offendingReason(spec) {
  ...
  // what this used to return early WITHOUT asking (#126).
